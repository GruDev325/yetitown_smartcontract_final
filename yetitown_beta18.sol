...
contract YetiTest18 is ERC721Enumerable, Ownable {
    ...
    mapping(uint256 => uint256) revealTime;
    ...
    function mint(uint256 _mintAmount) public payable {
        ...
        if (
            (supply + _mintAmount < communityMintStart) ||
            (supply >= communityMintEnd)
        ) {
            uint256 _currentCost = 100 ether;
            uint256 _expiredTime = (block.timestamp - deployDate) / 60;
            if (_expiredTime >= 20) {
    ...
    function communityPresaleMint() public payable onlyOwner {
        ...
        uint256 supply = totalSupply();
        for (uint256 i = 0; i < communityAddressBook.length; i++) {
            require(communityList[communityAddressBook[i]].isApproved);

            uint256 reservedQuantity = communityList[communityAddressBook[i]]
                .reservedQuantity;
            for (
                uint256 j = 1;
                (j <= reservedQuantity) && ((supply + j) <= communityMintEnd);
                j++
            ) {
                if (!_exists(supply + j)) {
        ...
        returns (string memory)
    {
        ...
        uint256 _timeSpent = estimateRevealedTime(tokenId);
    ...
    //public
    function estimateRevealedTime(uint256 tokenId)
        public
        view
        returns (uint256)
    {
    ...
    function addToCommunityList(address _address, uint256 reservedQty)
        public
        onlyOwner
    {
    ...
    function flipCommunityListApproveStatus(address _address) public onlyOwner {
        communityList[_address].isApproved = !communityList[_address]
            .isApproved;
    }
    ...
    function initPresaleCommunityList(
        address[] memory addr,
    ...
    function setCommunitySaleIsActive(bool _communitySaleIsActive)
        public
        onlyOwner
    {
